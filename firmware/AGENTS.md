...
- **Firmware serial monitor logs** are captured at `~/Workspace/viewfinitys9-brightness-control/firmware/monitor.log`. This file is produced by `idf.py monitor` over UART0 (`/dev/cu.usbserial-A5069RR4`). Read it to diagnose USB-CDC and command handling issues without needing the board connected live.
